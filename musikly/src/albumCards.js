...
musicData.map((curElem, ind) => {
  let queueCardElem = document.createElement("div");
  queueCardElem.className = "albumCard";
  queueCardElem.innerHTML = `
                  <img
                    src="${curElem.img}" class="${ind}"
                  />
                  <h5>${curElem.title}</h5>
                  <p>${curElem.artist}</p>`;
  document.querySelector(".albumSlider").appendChild(queueCardElem);
});
